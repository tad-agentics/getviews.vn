...
| M-6 | `6611af0`, `6f0d458`; **mode bundle — set SHA after commit** | `6611af0` — structured flop `analysis_headline` (**FlopHeadline** JSON in existing `TEXT` column, no migration); `6f0d458` — flop H1 + `IssueCard` title serif. **Client Win/Flop `Segmented`, `?mode=`, POST `mode`, pipeline `mode` + cache bypass, and `test_run_pipeline_respects_mode_override`** land in the **next** commit that touches `VideoScreen.tsx` / `useVideoAnalysis.ts` / `cloud-run/main.py` / `cloud-run/getviews_pipeline/video_analyze.py` / `cloud-run/tests/test_video_analyze.py` — replace this cell with that short SHA once pushed (e.g. ``git log -1 --oneline -- src/routes/_app/video/VideoScreen.tsx``). |
