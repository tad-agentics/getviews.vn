...
- **Presentation v1 (FE `VITE_REPORT_PRESENTATION_V2`, default off):** Retention chart + hook timeline strip + header lead chip (`bright_spot_signal` / flag-gated `lead_finding`) mount inside existing diagnosis blocks — no new top-level sections. Hybrid prose: static FE legends (0 budget) + BE findings per strip + structure-block closing in `script_structure.text`.
- **Synthesis flags (Cloud Run, default off):** `diagnosis_voice_lint_runtime` (soft-scrub `*_vi`), `diagnosis_lead_lever`, `diagnosis_wide_context` (wider `USER_EVIDENCE_DIGEST`), `diagnosis_salience_rank_only` (rank vs cap-drop), `diagnosis_proposed_findings` (two-tier findings, shadow/calibration-gated — no auto-promote).
