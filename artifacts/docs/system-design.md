...
- **Word budget (Path B + presentation v1, 2026-06-14):** «Nhịp & cắt» (`script_structure`) ≤120, hook (`hook_analysis`) ≤100, total report ~**430-520** (retry ceiling **600**) to give strip findings + measured signals room. Caps live in `diagnosis_quality.py` (code-enforced shorten-retry) + prompt copy in `diagnose_prompts.py` / `voice_guide.py` / `structure_axis_contract.py` — keep all in sync. LLM still surfaces **1–2 most decisive** signals by deviation magnitude (Loop B `predictive_strength` ranking deferred until calibration registers these signals).
- **Presentation v1 (FE `VITE_REPORT_PRESENTATION_V2`, default on):** Retention chart + hook timeline strip + header lead chip (`bright_spot_signal` / `lead_finding`) mount inside existing diagnosis blocks — no new top-level sections. Hybrid prose: static FE legends (0 budget) + BE findings per strip + structure-block closing in `script_structure.text`.
- **Synthesis flags (Cloud Run, default on):** `diagnosis_hook_leaderboard`, `diagnosis_comment_grounding`, `diagnosis_retention_structural`, `signal_calibration_adaptive`, `extraction_signals_v2`, `diagnosis_voice_lint_runtime`, `diagnosis_lead_lever`, `diagnosis_wide_context`, `diagnosis_salience_rank_only`, `diagnosis_proposed_findings`. Rollback per-flag via env. `EXTRACTION_AUDIO_DSP` remains off (Tier 3 deferred).
