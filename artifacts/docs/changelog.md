...
|---|---|---|---|---|
| Pipeline audit remediation | **CR-1** — Paginated `_load_all_existing_video_ids_sync` (`.range` 1000-row pages); one dedup snapshot per `run_batch_ingest`; `ingest_niche(..., existing_video_ids=)`. Fixes PostgREST 1000-row silent cap + per-niche refetch. | NO | Yes | 8e436ea |
