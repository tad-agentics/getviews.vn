      ...
      <div className="grid grid-cols-1 gap-px sm:grid-cols-2">
        {cells.map((c) => (
          <div key={c.title} className="bg-[color:var(--gv-paper)] p-4">
            <p className="gv-mono mb-2 text-[11px] tracking-wide text-[color:var(--gv-ink-3)]">{c.title}</p>
            <p className="gv-serif text-[24px] leading-none text-[color:var(--gv-ink)]">{c.finding}</p>
            <div className="mt-3">
              <PatternMiniChart cell={c} />
            </div>
            <p className="mt-2 text-sm text-[color:var(--gv-ink-3)]">{c.detail}</p>
          </div>
        ))}
      </div>
    </div>
