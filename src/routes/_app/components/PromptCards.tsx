    ...
    <div className="flex gap-2 overflow-x-auto pb-1 w-full" style={{ scrollbarWidth: "none" }}>
      {cards.map((text, idx) => (
        <button
          ...
          className="flex-shrink-0 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-xs text-[var(--ink)] transition-all duration-[120ms] hover:border-[var(--gv-ink)] hover:bg-[var(--surface-alt)] active:scale-[0.98] whitespace-nowrap"
        >
          {text}
        </button>
      ))}
