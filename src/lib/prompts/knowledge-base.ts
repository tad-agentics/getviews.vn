 ...
 * Used by: Gemini synthesis prompts (P0-3, P0-5)
 * Auto-referenced when: synthesis generates hook recommendations, diagnoses hook quality,
 *   or writes briefs with hook options.
 *
 * This file is STATIC — it ships with the codebase.
 * The CORPUS (video_corpus table) provides live data.
 * This file provides the structural vocabulary Gemini uses to classify and recommend.
 */
...
export const HOOK_CATEGORIES = {
  ...
  gia_soc: {
    ...
    best_niches: ['review_do_gia_dung', 'thoi_trang', 'tech', 'shopee_affiliate'],
  ...
  phan_ung: {
    ...
    best_niches: ['skincare', 'review_do_gia_dung', 'nau_an'],
  ...
  so_sanh: {
    ...
    best_niches: ['skincare', 'tech', 'thoi_trang'],
  ...
  bang_chung: {
    ...
    best_niches: ['giao_duc', 'tai_chinh', 'shopee_affiliate'],
    visual_pairing: 'Text overlay số liệu lớn, JetBrains Mono style. Screenshot kết quả thật.',
  },

  // ---- 5 additional hook types observed in Gemini output ----
  // These are NOT in Vietnamese creator educational content but Gemini
  // classifies real videos into them. Needed for corpus ↔ knowledge base parity.

  curiosity_gap: {
    name_vi: 'Tò Mò / Gợi Mở',
    name_en: 'curiosity_gap',
    description: 'Tạo khoảng trống thông tin — người xem phải xem hết để biết',
    templates: [
      'Bạn sẽ không tin điều này đâu...',
      'Mình thử [hành động] và kết quả...',
      'Cùng [người nổi tiếng] chấm điểm [sản phẩm] siêu hot trên TikTok nha',
      'Điều gì xảy ra khi [tình huống bất ngờ]?',
    ],
    mechanism_vi: 'Chạy vì: não bộ ghét thông tin thiếu → phải xem hết. Watch time cao nhất trong các loại hook.',
    best_niches: ['skincare', 'nau_an', 'giao_duc', 'tai_chinh'],
    visual_pairing: 'Mặt tò mò/hào hứng + sản phẩm che một phần. Reveal ở giữa video.',
  },

  bold_claim: {
    name_vi: 'Tuyên Bố Mạnh',
    name_en: 'bold_claim',
    description: 'Mở bằng tuyên bố gây sốc hoặc khẳng định mạnh — buộc người xem phản ứng',
    templates: [
      'Mai mốt mà em có mở quán cơm á thì em sẽ đưa cái món này vô thực đơn',
      '[Sản phẩm] này ngon nhức nách luôn mấy chị ơi',
      'Quán này vừa ngon vừa đông vừa rẻ — chắc chắn là quán này',
      'Điều này có thể gây tranh cãi nhưng...',
    ],
    mechanism_vi: 'Chạy vì: tuyên bố quá tự tin → người xem muốn xác minh. Comment đồng ý/phản đối → algorithm đẩy.',
    best_niches: ['nau_an', 'review_do_gia_dung', 'skincare', 'tai_chinh'],
    visual_pairing: 'Cận sản phẩm + mặt tự tin. Thành phẩm show trong 3s đầu.',
  },

  question: {
    name_vi: 'Câu Hỏi',
    name_en: 'question',
    description: 'Mở bằng câu hỏi từ khán giả hoặc câu hỏi relatable — tạo cảm giác trả lời cho chính người xem',
    templates: [
      'Sao [chủ thể] [kết quả tốt] thế chúng mày, tao [cố gắng] mà vẫn không được?',
      'Tại sao video bạn cứ [số] view mãi không lên?',
      'Có ai biết [sản phẩm] này mua ở đâu không?',
      '[Câu hỏi từ comment] — trả lời nè!',
    ],
    mechanism_vi: 'Chạy vì: câu hỏi relatable khiến người xem gật đầu → ở lại xem câu trả lời. Comment trả lời → engagement.',
    best_niches: ['skincare', 'giao_duc', 'tai_chinh', 'nau_an'],
    visual_pairing: 'Text overlay câu hỏi + mặt/sản phẩm. Hoặc screenshot comment.',
  },

  pain_point: {
    name_vi: 'Nỗi Đau',
    name_en: 'pain_point',
    description: 'Đánh thẳng vào vấn đề người xem đang gặp — tạo kết nối ngay lập tức',
    templates: [
      'Chỉ là bạn chưa [hành động] đúng với [yếu tố] của mình thôi',
      'Lông mi con ngắn và xơ xác quá, nhìn mà thấy tiếc quá đi',
      'Đừng [hành động sai] nữa!!!',
      'Đừng lướt qua nếu bạn đang [vấn đề] mà vẫn không [kết quả]',
    ],
    mechanism_vi: 'Chạy vì: người xem thấy chính mình → dừng lại ngay. Save rate cao vì muốn quay lại xem lại giải pháp.',
    best_niches: ['skincare', 'giao_duc', 'tai_chinh', 'thoi_trang'],
    visual_pairing: 'Mặt đồng cảm hoặc nghiêm túc. Text overlay mô tả vấn đề. Trước/sau transformation.',
  },

  trend_hijack: {
    name_vi: 'Đu Trend',
    name_en: 'trend_hijack',
    description: 'Bắt trend đang hot — nhắc tên creator/sản phẩm viral để hưởng reach',
    templates: [
      'Dạo này lướt TikTok thấy [người nổi tiếng] rần rần cái [sản phẩm] mới quá trời',
      'Ai cũng nói về [trend] — mình thử xem có đúng không',
      '[Trend viral] — phiên bản Việt Nam',
      'Reply to @[user]: [trả lời trend]',
    ],
    mechanism_vi: 'Chạy vì: algorithm đang đẩy keyword này → video của mình được hưởng lây. Timing quan trọng hơn chất lượng.',
    best_niches: ['thoi_trang', 'skincare', 'giai_tri', 'nau_an'],
    visual_pairing: 'Chia đôi màn hình với video gốc hoặc nhắc tên creator. Dùng sound trending.',
  },
} as const;
...
export const COMMERCE_VIDEO_STRUCTURE = {
  ...
  // ALL content formats — must match classifyFormat() output in video-corpus-quality-rules.md
  formats: [
    { format: 'review', name_vi: 'Review sản phẩm', hook_pairing: ['phan_ung', 'boc_phot', 'so_sanh', 'curiosity_gap'] },
    ...
    { format: 'tutorial', name_vi: 'Hướng dẫn', hook_pairing: ['huong_dan', 'bang_chung', 'pain_point'] },
    { format: 'grwm', name_vi: 'GRWM', hook_pairing: ['ke_chuyen', 'pov', 'pain_point'] },
    { format: 'mukbang', name_vi: 'Mukbang / ASMR ăn', hook_pairing: ['bold_claim', 'curiosity_gap', 'ke_chuyen'] },
    { format: 'recipe', name_vi: 'Nấu ăn / Công thức', hook_pairing: ['bold_claim', 'huong_dan', 'ke_chuyen'] },
    { format: 'storytelling', name_vi: 'Kể chuyện', hook_pairing: ['ke_chuyen', 'pain_point', 'bang_chung'] },
    { format: 'pov', name_vi: 'POV', hook_pairing: ['pov'] },
    { format: 'outfit_transition', name_vi: 'Biến hình outfit', hook_pairing: ['pov', 'gia_soc', 'trend_hijack'] },
    { format: 'vlog', name_vi: 'Vlog / Daily life', hook_pairing: ['ke_chuyen', 'curiosity_gap'] },
    { format: 'dance', name_vi: 'Dance / Trend âm nhạc', hook_pairing: ['trend_hijack'] },
    { format: 'before_after', name_vi: 'Trước/Sau', hook_pairing: ['phan_ung', 'bang_chung', 'pain_point'] },
    { format: 'faceless', name_vi: 'Không mặt (tay + voice)', hook_pairing: ['huong_dan', 'gia_soc'] },
    { format: 'other', name_vi: 'Khác', hook_pairing: [] },
  ],
} as const;

// ============================================================
// NICHE ID ↔ KEY MAPPING — connects Supabase niche_taxonomy to knowledge base
// ============================================================

export const NICHE_ID_MAP: Record<number, string> = {
  1: 'review_do_gia_dung',
  2: 'skincare',
  3: 'thoi_trang',
  4: 'nau_an',
  5: 'mmo',
...
export const NICHE_KEY_TO_ID: Record<string, number> = Object.fromEntries(
  Object.entries(NICHE_ID_MAP).map(([id, key]) => [key, Number(id)])
);
 ...
 */
export function getHookNameVI(category: string): string {
  if (category in HOOK_CATEGORIES) {
    return HOOK_CATEGORIES[category as HookCategory].name_vi;
  }
  return category; // fallback: return raw key
}

/**
 * Returns top hook categories for a niche — accepts EITHER string key or numeric ID
 */
export function getTopHooksForNiche(nicheKeyOrId: string | number): HookCategory[] {
  const key = typeof nicheKeyOrId === 'number'
    ? NICHE_ID_MAP[nicheKeyOrId]
    : nicheKeyOrId;
  return NICHE_HOOK_GUIDANCE[key]?.top_hooks ?? ['canh_bao', 'phan_ung', 'huong_dan'];
}
 ...
 */
export function getMechanismVI(category: string): string {
  if (category in HOOK_CATEGORIES) {
    return HOOK_CATEGORIES[category as HookCategory].mechanism_vi;
  }
  return '';
}

/**
 * Returns best hook-format pairings for a given format
 */
export function getHookPairingsForFormat(format: string): string[] {
  const entry = COMMERCE_VIDEO_STRUCTURE.formats.find(f => f.format === format);
  return entry ? [...entry.hook_pairing] : [];
}

/**
 * Returns niche string key from numeric ID
 */
export function getNicheKey(nicheId: number): string | undefined {
  return NICHE_ID_MAP[nicheId];
}

/**
 * Returns niche numeric ID from string key
 */
export function getNicheId(nicheKey: string): number | undefined {
  return NICHE_KEY_TO_ID[nicheKey];
}
...
export function isEnglishLoanword(word: string): boolean {
  const loanwords = CREATOR_TERMS.english_loanwords as readonly string[];
  return loanwords.includes(word.toLowerCase()) || loanwords.includes(word);
}

/**
 * Returns all commerce detection keywords (for sharing with detectCommerce classifier).
 * Source of truth: SHOPEE_HOOKS.cta_phrases + TIKTOK_SHOP_HOOKS + CREATOR_TERMS.commerce
 */
export function getCommerceKeywords(): string[] {
  return [
    ...SHOPEE_HOOKS.cta_phrases,
    ...TIKTOK_SHOP_HOOKS.urgency,
    ...TIKTOK_SHOP_HOOKS.trust,
    ...TIKTOK_SHOP_HOOKS.shipping,
    ...Object.keys(CREATOR_TERMS.commerce),
  ];
}
