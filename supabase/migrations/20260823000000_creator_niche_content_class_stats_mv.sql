-- Wave 3c — creator_niche × content_class stats MV for morning ritual class anchor.
-- Refreshed after content_class_intelligence refresh (nightly post-ingest).

BEGIN;

CREATE MATERIALIZED VIEW IF NOT EXISTS creator_niche_content_class_stats AS
SELECT
  j.creator_niche_id,
  j.content_class_id,
  j.is_primary,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_creator_niche_cc_stats_pk
  ON creator_niche_content_class_stats(creator_niche_id, content_class_id);

CREATE INDEX IF NOT EXISTS idx_creator_niche_cc_stats_niche_sample
