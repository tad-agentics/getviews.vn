-- Extend upsert_video_corpus_batch for corpus ingest criteria v1 columns.
-- Without this, batch RPC ignores boost_attribution / reference_eligible /
-- ingest_relaxation_tier even though Python sends them on each row.
...
COMMENT ON FUNCTION upsert_video_corpus_batch(JSONB) IS
  'Provenance-safe batch upsert for corpus_ingest. Persists boost_attribution, '
  'reference_eligible, ingest_relaxation_tier (20260730000000).';
