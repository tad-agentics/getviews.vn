-- Repair: ensure PR2 corpus DDL ran on databases where migration version
-- ``20260511000000`` previously collided with ``video_shots_sql_backfill``
-- (same timestamp). Those remotes can have ``schema_migrations`` satisfied
-- for ``20260511000000`` while ``video_corpus.content_class_id`` was never
-- added. This file repeats the idempotent body of
-- ``20260511000000_two_axis_niche_pr2_corpus.sql`` so ``db push`` can
-- proceed to materialized views that depend on the column.
--
-- Safe when PR2 already applied: ``IF NOT EXISTS`` / ``CREATE OR REPLACE`` /
-- ``UPDATE … WHERE content_class_id IS NULL``.
