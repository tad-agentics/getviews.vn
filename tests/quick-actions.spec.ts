...
test("quick-action: Soi Kênh Đối Thủ modal navigates to Studio channel block (B.3.4)", async ({ page }) => {
  await page.goto("/app");
  await page.waitForLoadState("domcontentloaded");
  const newChatButton = page
    .getByRole("button", { name: /new chat|chat mới|\+/i })
...
test("home: Tìm KOL quick action navigates to /app/kol (B.2.3)", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Khám Video flop" }).first()).toBeVisible({
    timeout: 20_000,
  });
