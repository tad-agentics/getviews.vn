...
**Supabase Edge Functions:** Deno modules in `supabase/functions/`. Deploy via `supabase functions deploy [name]`. Migrations in `supabase/migrations/` — both Supabase MCP (remote apply) and local SQL file must be written; they must never drift. Regen types with `supabase gen types typescript --project-id <ref> > src/lib/database.types.ts` after schema changes. **Destructive migrations (DROP/RENAME):** follow `artifacts/docs/destructive-migration-checklist.md` — grep all three runtimes before the object dies. **New/changed pg_cron jobs:** register them in `expected_cron_jobs` in the same migration (the daily `cron-inventory-watch` alerts on drift).
...
- **Answer sessions are the only session model.** `answer_sessions` + `answer_turns` handle every intent (structured video/channel diagnosis ①③④ **and** the text intents that used to be "chat"). The legacy chat model — `chat_sessions`/`chat_messages` tables, Cloud Run `intent.py`, and `POST /stream` — was fully removed in **Phase C** (migration `20260830000001_phase_c_drop_chat_sessions.sql`). `history_union` now surfaces answer sessions only. The sole paid SSE path is `POST /answer/sessions/{id}/turns`; `POST /channel/diagnose` is the other paid path — **both** take the TD-3 `begin_processing` per-user lock before deducting credits.
...
- **Axes:** `creator_niches` (16 active UX buckets; `pets_home` retired) and `content_classifications` (82, incl. 5 `carousel_format_*` linked primary to every niche) joined by `creator_niche_content_classes` (M:N, `is_primary`). Query live counts before quoting a number — they grow.
- **Legacy bridge:** `video_corpus.niche_id` was **dropped** (Phase C — `20260822000001_phase_c_drop_video_corpus_niche_id.sql`); legacy-niche corpus filtering now uses `video_corpus.ingest_loop_niche_id` (the canonical cohort uses `content_class_id`). Python `legacy_niche_id_for_creator_niche()` and TypeScript `legacyNicheIdForCreatorNiche()` (creator_niche → legacy niche id) **must stay identical** — CI-enforced by `cloud-run/tests/test_niche_parity_ts.py`, which parses the TS source and diffs both maps (incl. the retired-id aliases).
- **Batch ingest resolver:** `NICHE_RESOLVER_MODE` = `shadow` (code default if unset) or `route`. **Production (2026-05-17+):** `route` on batch + user pods. In **route**, high-confidence HI-9 output + junction can override `niche_id` / `content_class_id`; shadow telemetry columns still populated. Rollback: `NICHE_RESOLVER_MODE=shadow` (see `artifacts/docs/two-axis-niche-cutover-runbook.md` Part B).
...
- **Content-class pivot (Phase 0–4, promoted 2026-05-21):** Production defaults — `CORPUS_SCORE_COHORT=class`, `CORPUS_INGEST_LOOP=class`, `LIVE_COHORT_CLASS_FIRST=true`, `CORPUS_WRITE_NICHE_ID=false`, `REFRESH_NICHE_INTELLIGENCE_MV=false`, `VITE_CORPUS_BROWSE_CLASS_FIRST` + `VITE_CORPUS_BROWSE_CLASS_ONLY` on (FE opt-out with `"false"`). Cohort canonical = `(content_class_id, creator_tier)`. ACQE nightly + class MV refresh still run; legacy `niche_intelligence` MV refresh skipped. See `artifacts/docs/content-class-pivot-metrics.sql` and `system-design.md` §9.
...
- Server-only (no `VITE_`): `GEMINI_API_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_JWT_SECRET`, `ENSEMBLEDATA_API_KEY`, `PROXY_URL`, `BATCH_SECRET`, `BATCH_SERVICE_BASE_URL` (set on the user pod so its `batch_proxy` can forward to batch), `PAYOS_*`, `RESEND_API_KEY`, `R2_*`. **Daily API budgets (Cloud Run batch pod):** `ED_BATCH_DAILY_REQUEST_MAX` (VN TikTok EnsembleData cap; `0` = unlimited) and `TIKHUB_DOUYIN_DAILY_REQUEST_MAX` (Douyin TikHub cap; default **250** in `cloud-run/getviews_pipeline/config.py`; `0` = unlimited). Both raise 503 on `/batch/*` ingest routes when exhausted so pg_cron retries after UTC rollover.
