...
- **Legacy bridge:** `video_corpus.niche_id` was **dropped** (Phase C — `20260822000001_phase_c_drop_video_corpus_niche_id.sql`); legacy-niche corpus filtering now uses `video_corpus.ingest_loop_niche_id` (the canonical cohort uses `content_class_id`). Python `legacy_niche_id_for_creator_niche()` and TypeScript `legacyNicheIdForCreatorNiche()` (creator_niche → legacy niche id) **must stay identical**.
